...
- **2026-06-08 (xác minh sống guard `causedByJobId` bằng test stub e2e + dọn nốt `saveVideoContent`)**:
  - **Test xác minh trực tiếp trên production** (đã xin phép user trước khi chạy, vì script chèn/xoá `videos`/`jobs` thật — đúng convention "stub rồi dọn"): tạo 1 video + 1 job P2 stub ở trạng thái `done, consumed_at=NULL`, gọi `process-jobs` lần 1 (handler chạy xong, tạo job P3 #71 mang `metadata.causedByJobId=70`, đóng dấu `consumed_at`) → **chủ động xoá `consumed_at` về NULL** để mô phỏng đúng trạng thái DB nếu handler crash trước `markJobConsumed` → gọi `process-jobs` lần 2 (handler chạy lại từ đầu) → kiểm tra: **vẫn chỉ có đúng 1 job P3** (#71, không sinh thêm #72) — `findJobByCause` đã chặn đúng bản sao. **PASS**. Đã dọn sạch toàn bộ dữ liệu stub (video #29, job #70/#71) + xoá file script test.
  - **Dọn nốt phần còn thiếu của Finding #1 — `saveVideoContent` trùng lặp khi handler chạy lại**: thay vì thêm migration cột `sourceJobId` (nặng, cần đụng schema production), dùng tín hiệu sẵn có: `job.result` luôn **giống hệt byte-for-byte** giữa lần chạy gốc và lần chạy lại sau crash (worker ghi 1 lần, không sửa đổi sau đó), trong khi 1 lần lặp lại hợp lệ (vd vòng `needs_retry` gọi lại P3 thật) sẽ tạo ra output MỚI từ ChatGPT. → `saveVideoContent` giờ kiểm tra tồn tại bản ghi khớp chính xác `(videoId, stage, output)` trước khi insert — khớp thì trả về bản cũ, không khớp thì lưu như bình thường (giữ đúng lịch sử audit cho các lần lặp thật). Không cần đổi schema, không rủi ro migration. `tsc --noEmit` sạch. Commit `0c97bd6`, đã deploy production.
  - **Tổng kết Finding #1**: cả 2 nhánh nguy hiểm nhất (job downstream trùng qua `enqueueStage`, và `video_content` trùng qua `saveVideoContent`) đã được vá và xác minh sống. Chỉ còn lại 1 góc nhỏ chưa xử lý triệt để — `handleP1Done`'s vòng lặp tạo `videos` mới có thể trùng nếu crash giữa chừng (chỉ được `isDuplicateTopic` giảm thiểu một phần, vì mỗi lần chạy lại sinh ra `video.id` khác nên không khớp được `causedByJobId`). Việc làm `createVideo` idempotent theo "dấu vân tay" topic gốc để lại như việc cần làm sau — đụng vào logic anti-duplication nên cần thiết kế cẩn thận, không vá vội.
