...
import { listNeedsAttentionDrEpisodes } from "@/lib/db/repo/episodes";
import { listFailedDrJobsByEpisode } from "@/lib/db/repo/jobs";
...
export default async function NeedsAttentionPage() {
  const episodes = await listNeedsAttentionDrEpisodes();

  const episodesWithJobs = await Promise.all(
    episodes.map(async (ep) => ({
      episode: ep,
      failedJobs: await listFailedDrJobsByEpisode(ep.id),
    })),
  );
    ...
    <>
      <div>
        ...
        <p className="mt-1 text-[15px] leading-relaxed text-[#6E6E73] dark:text-[#AEAEB2]">
          Episodes stuck in pipeline — review errors and retry failed jobs.
        </p>
      </div>

      {episodesWithJobs.length === 0 ? (
        <Card className="border-black/[.08] bg-white shadow-none rounded-xl dark:border-white/[.10] dark:bg-[#1C1C1E]">
          <CardContent className="p-12 text-center text-[15px] text-[#AEAEB2]">
            All clear — no episodes need attention.
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-6">
          {episodesWithJobs.map(({ episode, failedJobs }) => {
            const scene = episode.sceneInput as SceneInput | null;
            const title = episode.ytTitle ?? scene?.scene_name ?? `Episode #${episode.id}`;
            return (
              <Card key={episode.id} className="border-[#FF3B30]/20 shadow-none rounded-xl dark:border-[#FF3B30]/20 dark:bg-[#1C1C1E]">
                <CardContent className="p-5 space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-[13px] text-[#AEAEB2]">
                        Episode #{episode.id} · Updated {formatRelative(episode.updatedAt)}
                      </p>
                    </div>
                    <div className="flex shrink-0 items-center gap-2">
                      <RetryVideoButton videoId={episode.id} />
                      <Badge className={`text-[12px] ${statusBadgeClass(episode.status)}`}>
                        {episode.status}
                      </Badge>
                    </div>
                  </div>
                  ...
                  {failedJobs.length > 0 ? (
                    ...
                    <p className="text-[13px] text-[#AEAEB2]">No failed jobs — use Retry to resume from the next pipeline stage.</p>
                  )}
                </CardContent>
              </Card>
            );
